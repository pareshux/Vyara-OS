...
>
> **Last updated:** 2026-06-21 (INT-014 Slice 2 shipped — Owner Dashboard /owner finance depth (per "money first" feedback, reorders Blueprint plan): Sections 3 Receivables ageing (4-bucket stacked bar + per-bucket cards, drill into /collections), 4 Top debtors (top 10 by ₹ outstanding, link → Customer 360), 5 Cash movement (receipts in 30d + payment-mode split + best day; OUT as honest gap marker → FIN-014), 6 PTP coverage (% covered + due-this-week + dishonoured 30d). Read-model extended with 2 new queries (receipts_30d w/ payment_mode + received_at, promise_to_pay open + dishonoured), buyer-name resolution merged for overdue + debtors (no extra query). AI brief context extended with receivables_depth block (top 3 debtors, ageing buckets, PTP coverage, cash-in 30d) — prompt v1 → v2 with cache-key versioning to invalidate stale briefs. Slices 1–2 shipped; Slices 3–5 = Revenue+Ops, Field+People, Drill-downs.)
