...
| DEL-017 | Procurement: Goods Receipt Note (GRN) + RTV | Must-have post-C#2 | ✅ Partial (schema) | Procurement P1α · `goods_receipt_note` + `goods_receipt_note_line` tables shipped in migration 0054 (status enum: draft / posted / cancelled; QC enum: not_required / pending / accepted / rejected / partial_accept; per-line qty_received / qty_accepted / qty_rejected with CHECK that accepted+rejected ≤ received; batch_no, expiry_date, rejection_reason captured). Code template + sequence + auto-number trigger wired (Vyara `VT-GRN-*`, Raj `RA-GRN-*`). **Consumer (server actions + UI) lands in P1β** — GRN posting writes `stock_movement` rows with `related_entity_type='goods_receipt_note', related_entity_id=grn.id` via the existing polymorphic FK; no further schema change required when 1β ships. |
