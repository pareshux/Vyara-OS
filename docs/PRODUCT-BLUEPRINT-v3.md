...
| DEL-020 | Procurement: RFQ + Comparative Statement | Should-have | ✅ (multi-PO from one CS → P4δ) | Procurement P4β + P4γ · migration 0069 ships 4 tables: `request_for_quotation` (header — source_pr_ids JSONB array for consolidation, response_deadline, 6-state status enum draft/sent/quotes_collected/cs_finalised/po_raised/cancelled, linked_po_id), `request_for_quotation_line` (with optional source_pr_line_id for traceability), `request_for_quotation_vendor` (invited vendors + vendor's quote no/date/validity/payment-terms/delivery-terms captured at vendor level), `request_for_quotation_response` (per-vendor per-line quote: rate/discount/GST/delivery-days/computed taxable+total/L1 flag/selected flag/override reason). purchase_order extended with source_pr_id + source_rfq_id columns for full traceability. `lib/actions/rfqs.ts` ships 7 actions: createRfq (validates min-2-vendor invitation), sendRfq, cancelRfq, recordVendorRfqResponse (upsert per-line responses; computes taxable_value + amount_total; auto-flips RFQ → quotes_collected when all vendors responded), finaliseCs (clears prior selections, computes L1 per line as lowest amount_total, sets is_selected + per-line override reasons, flips RFQ → cs_finalised), listRfqs + getRfq, plus 2 form pickers (listApprovedPrsForRfq + getPrLinesForRfq for consolidation). `lib/actions/purchase-orders.ts` extended with getPrForPoPrefill + getRfqForPoPrefill and from_pr_id + from_rfq_id params on createPurchaseOrder (sets PR.linked_po_id + PR.status='po_raised' OR RFQ.linked_po_id + RFQ.status='po_raised' on PO save). UI: `/procurement/rfqs` list with 7-status filter + per-row response counts. `/procurement/rfqs/new` two-mode (blank or source PRs via ?pr=id1,id2 multi-select) with project + cost-center + response-deadline + min-2-vendor checkbox grid + dynamic line table (lines auto-fill when PRs selected, fully editable). `/procurement/rfqs/[id]` detail with header pills + meta + sourced-from-PRs chip + linked-PO chip + "Open CS" button when responses exist + 5-column lines table + vendors-invited card with per-vendor responded-status badge + "Record/Edit response" link per vendor. `/procurement/rfqs/[id]/responses/new?vendor=X` form to capture one vendor's per-line quote (rate / discount / GST / delivery-days / notes) with live taxable + total preview; edit-mode pre-populates existing data. **`/procurement/rfqs/[id]/cs` Comparative Statement** — vendor × line matrix with sticky vendor header, L1 (lowest landed cost) auto-highlighted in amber, click any cell to pick that vendor for the line (emerald highlight), override-reason input required when picking non-L1, Finalise CS button with confirm dialog flagging multi-vendor selection (v1 creates PO for the most-picked vendor only — multi-PO from one CS lands in P4δ), per-vendor grand-total summary card. "Create PO from CS" CTA appears after finalisation → routes to /procurement/orders/new?from_rfq=X&vendor=Y with selected lines + rates pre-filled. PR detail extended with "Raise PO from this PR" sky-blue CTA on approved PRs (replaces the earlier P4β-marker hint). PO new page extended with ?from_pr= and ?from_rfq=&vendor= path; form prefill chain: rfqPrefill > prPrefill > blank. Procurement landing extended with "RFQ + Comparative Statement" Live ✓ link. **Deferred to P4δ:** Multi-PO from one CS (when different vendors selected across lines, create one PO per vendor with the lines they won). |
| DEL-021 | Procurement: Job Work + ITC-04 | Nice-have | ✅ | Procurement P6 lite · migration 0071 ships `job_work_challan` schema with sequence + render_tenant_code trigger + tenant code templates VT-JWC-* / RA-JWC-*. `lib/actions/job-work.ts` ships 6 actions: createJobWorkChallan (snapshots vendor GSTIN at challan time so ITC-04 stays correct even if vendor details change later), recordJobWorkReturn (atomic partial-return tracking; flips status to partly_received / fully_received based on remaining qty), cancelJobWorkChallan, listJobWorkChallans + getJobWorkChallan + listJobWorkersForPicker, plus createJobWorkChallanForm wrapper. UI: `/procurement/job-work` list with 4-status filter + KPI strip (qty sent / pending / received / scrap) + `<QuarterlyItc04Button>` FY+Q1-Q4 picker dialog. `/procurement/job-work/new` form with vendor picker showing GSTIN inline + process_nature dropdown (machining / cutting / coating / galvanising / powder_coating / assembly / welding / polishing / heat_treatment / plating / other) + 1-year inputs / 3-year capital reminder. `/procurement/job-work/[id]` detail with movement progress bar (received emerald + scrap rose) + record-return form for in-flight challans + overdue banner. ITC-04 CSV export at `/api/procurement/job-work/export-itc04?fy=2026&q=1` per ITC-04 form Table 4 + 5A/B intent (GSTIN with URP fallback for unregistered job workers / Challan No / Date / Process / HSN / Description / UQC / Qty Sent + Rate + Value / Qty Returned + Scrap / Return Date / Status / Days Since Challan). Sample data (0072): Vyara VT-JWC-2026-0001 (pigment fine grinding, partly received with scrap) + Raj RA-JWC-2026-0002 (panel powder coating in-flight) + RA-JWC-2026-0003 (busbar cutting fully received). |
| DEL-022 | Procurement: Blanket PO + Rate Contract + Release Orders | Nice-have | ✅ Partial (multi-line release in v2) | Procurement P6 lite · migration 0071 ships `blanket_po` schema with annual qty_cap + rate (value_cap auto-computed as STORED), validity window, 5-state lifecycle (draft / active / exhausted / expired / cancelled), `qty_released` drawdown tracker, sequence + render_tenant_code trigger + tenant code templates VT-BPO-* / RA-BPO-*. `purchase_order.blanket_po_id` FK column added so any PO can be linked back as a release order. `lib/actions/blanket-pos.ts` ships 7 actions: createBlanketPo, cancelBlanketPo (draft/active only), listBlanketPos (with release_po_count derived from a single bulk count query — no N+1), getBlanketPo (header + release POs with per-PO qty drawn from line aggregation), recomputeBlanketReleased (sums non-cancelled child PO line quantities + flips status to `exhausted` when drawn ≥ cap, back to `active` when reduced), listBlanketPosForPicker, createBlanketPoForm wrapper. `createPurchaseOrder` extended with `blanket_po_id` param + auto-recompute hook after PO insert when set. UI: `/procurement/blanket-pos` list with 5-status filter + drawdown stacked-bar visualization per row (emerald < 75% / sky 75-100% / amber ≥ 100%) + release-PO count badge + remaining qty + value. `/procurement/blanket-pos/new` form with FY-default validity (Apr 1 → Mar 31). `/procurement/blanket-pos/[id]` detail with status pill + "Release PO from blanket" sky CTA when active + qty_remaining > 0 + 3-stat drawdown card (Cap / Released / Remaining) + locked-rate + value-cap chips + expired banner + release-POs table with per-PO qty drawn. "Release PO from blanket" routes to `/procurement/orders/new?blanket=<id>`; the new PO page reads the param via `getBlanketPo`, defaults vendor to blanket's vendor, pre-fills one line with the blanket's description + locked rate + unit + HSN, and passes `blanket_po_id` so drawdown auto-updates on save. Sample data (0072): Vyara VT-BPO-2026-0001 (10,000 bags Ambuja cement @ ₹360 for FY26-27) + Raj RA-BPO-2026-0002 (15,000 mtr Polycab LT cable @ ₹825) + RA-BPO-2026-0003 (500 nos Schneider MCCB 100A @ ₹18,500 with 120 already drawn — exercises partial-drawdown visualization). **Deferred:** multi-line release orders (v1 limits to 1 line per release; multi-line trivial when a customer asks since the PO form already supports it), multi-blanket consolidated drawdown across vendors. |
| DEL-023 | Procurement: Import procurement (Bill of Entry, FX, customs) | Nice-have | ✅ Lite (BoE fields shipped; full LC + import-PO flow deferred) | Procurement P6 lite · migration 0071 extends `purchase_order` with 6 columns: `bill_of_entry_no`, `bill_of_entry_date`, `customs_duty` (₹), `cif_inr_rate` (INR per unit foreign currency), `port_of_loading`, `port_of_discharge`. `createPurchaseOrder` accepts the 6 params + persists. Captures the document-trail data an importer's CHA produces; rendering on PO detail + a dedicated "import" PO mode + LC management + full FX gain/loss tracking land when a tenant asks. No sample import PO seeded — exercising would require backdating a real BoE workflow which doesn't belong in a lite slice. **Deferred:** dedicated import-mode PO form section, BoE-instead-of-GRN ingestion flow, IGST-on-customs computation, LC + bank-guarantee management, FX hedging gain/loss tracking, customs broker workflow. |
| DEL-024 | Procurement: Vendor performance scorecards | Should-have | ✅ | Procurement P6 lite · migration 0071 ships `vendor_scorecard_v` PG view (security_invoker=true per the 0047 cross-tenant fix) — one row per vendor × Indian FY where any procurement activity exists. Aggregates over PO + GRN + GRN line + vendor_bill, exposing: po_count + po_value + po_fulfilled + po_cancelled, grn_count + grn_on_time + grn_with_eta + on_time_pct, qty_received + qty_accepted + qty_rejected + acceptance_pct, bill_count + bill_value + approved_bill_count + approved_bill_value + mismatched_count + outstanding_total. Both percentage metrics are nullable so the read-model surfaces "not measurable yet" (no GRN with ETA, no GRN qty received) rather than misleading 100%. `lib/read-models/vendor-scorecard.ts` reads the view, computes per-vendor grade (A/B/C/unrated) per simple thresholds: A = on-time ≥ 90% AND acceptance ≥ 98% AND zero mismatched bills; B = on-time ≥ 70% AND acceptance ≥ 95%; C = anything below (with activity); unrated when no GRN with ETA and no qty received yet. Indian-FY current-year detection (`currentFyStartYear`) so the default view is FY 2026-27 (today). `/procurement/vendors/scorecards` page with 4-KPI strip (vendor count / total spend / avg on-time / avg acceptance) + grade rollup card (A/B/C/MSME counts + spend) + per-vendor sortable table (rank with trophy icons for top 3, MSME badge inline, grade pill, PO + GRN + bill counts, on-time + acceptance colored by threshold, mismatched count + outstanding ₹). FY toggle in header (current + 2 prior). Empty state when no activity exists yet. Vendors with no measurable metric (on_time_pct null / acceptance_pct null) render `—` rather than fake numbers. Procurement landing extended with "Vendor scorecards" Live ✓ link. **Architectural call:** read-model is pure aggregation — no `vendor_score` denormalized column. Computing on read is cheap (one view query); refreshing as POs / GRNs / bills are posted is automatic since the view re-evaluates. When scorecard frequency matters (e.g. dashboard widget that polls), we'll add a materialized view + refresh trigger; v1 keeps the schema clean. |
