...
>
> **Last updated:** 2026-06-21 (INT-014 Slice 3.1 — AI brief redesigned per user feedback "too text heavy, hard to focus". Old 3-column wall (~250 words across opportunities/risks/recommendations) replaced with: severity chip + one-sentence headline + up to 3 action chips that drill to list pages with `?q=<entity_search>`. Schema: `{ health, headline (≤22w), actions[ {label (≤10w), target, search} ] }`. Prompt v3 → v4 with explicit good/bad chip examples and verb-first style rules. Component swap to chip row. ~80% less text. Conversational AI agent (INT-009) lifted from 💭 to 📋 as the natural "tell me more" companion. Slices 1–3.1 shipped; Slices 4–5 = Field+People, Drill-downs.)
...
| INT-009 | Conversational query — tool-use agent over read-models | Should-have | 📋 | Lifted from 💭 to 📋 as the natural "tell me more" companion to the trimmed Owner Brief (Slice 3.1). Tool-use agent pattern (NOT raw LLM-to-SQL): predefined safe tools wrapping the existing read-models + masters (look_up_firm, recent_payments, lead_count_in_period, find_overdue_invoices, find_quote_status, field_visits_for_firm, …). Read-only v1 — no writes from chat (otherwise scope explodes past approval gates). Mandatory "I don't have that" path so the model can't invent. Cost: ~₹2-5 / message at Anthropic Sonnet pricing; cache by `(tenant, normalised_query)` for 5-10 min. UX surface TBD: sidebar panel on /owner or global Cmd+K overlay. |
