...
>
> **Last updated:** 2026-06-22 (Constitution v2 → v3 amendment + Raj Avinsys cross-industry demo plan locked. **Constitution amended** from "vertical SaaS for made-to-order building-materials manufacturers" to "modular Business Operating System for manufacturing / contracting / distribution / service companies; building-materials-first launch, cross-industry by configuration." Trigger: pitching Raj Avinsys Pvt. Ltd. (electrical EPC + panel manufacturing + AMC, Gujarat) forced the recognition that the Blueprint (v3) was already shaped horizontally — the Constitution was the document out of sync. Year-1 success criterion changed from "second similar tiles/pavers maker in ≤8 weeks" to "customer in a different industry in ≤8 weeks." Principles #0–#11 unchanged (always industry-neutral); only §5 (in/out-of-scope language broadened to include procurement-for-EPC; RMC-shape still requires deliberate decision) and §10 (cross-industry expansion now an explicit year-1 design constraint) were rewritten. CLAUDE.md preamble + Current step + Platform discipline test + Foundational audit Q7 all updated to reference Constitution v3 + Raj as the live test customer. **Raj demo plan locked at "Full" depth (~3 weeks across 5 phases)**: Phase 1 = provision raj-avinsys tenant + Raj-shaped masters (16-stage EPC pipeline, electrical activity-type vocabulary, industrial/consultant/OEM relationship types) + /demo landing page with two "Sign in as…" buttons; Phase 2 = mock data; Phase 3 = CS-001 minimum-viable complaint module (also a Raj-demo requirement, gets built now); Phase 4 = CS-009 AMC contracts; Phase 5 = drawing-approval gate + milestone-billing schedule (10/30/30/20/10). Phased-builds rule applies — walk each phase in-browser before starting the next. The demo doubles as the **Customer-#2 onboarding rehearsal** — same code path a real second customer would take. New feedback memory `feedback-cross-industry-by-configuration` records the binding principle.)
