...
>
> **Last updated:** 2026-06-21 (INT-014 Slice 3 shipped — Owner Dashboard /owner revenue + ops: Sections 7 Pipeline funnel (4 stages with conversion %s, period-coupled), 8 Win rate + cycle (accepted/rejected ratio + avg quote→close days + top loss reasons + losses-without-reason hygiene flag), 9 Top reps (top 5 by closed ₹ + personal win rate, trophy/award icons), 10 Operations (dispatch counts + in-transit + avg cycle + gap markers for DEL-007 on-time % and stock at risk). Read-model adds 11 new queries (open leads count, won/lost leads in period, sent/accepted/rejected quotes in period, dispatch stage rollup, dispatches in period, delivered in period, in-transit point-in-time, lead_loss_reason master). Rep name resolution via new user_profile lookup. AI brief context extended with revenue_depth block (funnel + conversions + win rate + top reps + ops) — prompt v2 → v3 with cache-key versioning. Slices 1–3 shipped; Slices 4–5 = Field+People, Drill-downs.)
