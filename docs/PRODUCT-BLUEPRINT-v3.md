...
>
> **Last updated:** 2026-06-20 (Customer 360 Slice 1.6 — REL-009 still ✅ Partial. Page restructured: header card + tabs below (Overview default + Projects + Contacts). KPI strip + AI insights placeholder + Notes in Overview. Read-model gains `contacts[]` + `kpis` aggregates. Slice 2 tabs (Quotes / Orders / Invoices / Collections) now slot in via one query + one TabsContent each.)
