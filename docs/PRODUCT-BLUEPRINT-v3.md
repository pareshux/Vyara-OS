...
| DEL-015 | Procurement: Purchase Requisition (PR / Indent) | Should-have | 📋 P4 | Per PO decision, PR lands in P4 with RFQ — Tier-2 firms don't do formal PRs today and demand-to-PO traceability arrives more naturally bundled with RFQ + CS. |
...
| DEL-017 | Procurement: Goods Receipt Note (GRN) + RTV | Must-have post-C#2 | ✅ (RTV → P1γ) | Procurement P1α (schema) + P1β (consumer) · `goods_receipt_note` + `goods_receipt_note_line` schema in migration 0054; consumer shipped in P1β. `lib/actions/goods-receipt-notes.ts` ships 5 server actions: `createGoodsReceiptNote` (greedy-receive defaults qty to PO line's qty_pending; per-line qty_rejected with required reason; QC status capture; optional `post_immediately` skips draft state), `postGoodsReceiptNote` (the atomic 4-step state advance: flip GRN status → increment po_line.qty_received + qty_rejected per accepted/rejected qty → recompute parent PO status (all lines fulfilled = received; any received = partly_received; else fallback sent) → write stock_movement rows with `movement_type='receipt' + reason_code='purchase' + related_entity_type='goods_receipt_note'` for each accepted line where product_id is set; ad-hoc product-null lines skip stock impact; best-effort rollback on intermediate failure mirroring complaints.ts pattern), `cancelGoodsReceiptNote` (draft-only — posted GRNs need RTV in P1γ), `listGoodsReceiptNotes` + `getGoodsReceiptNote` + `getPoForReceive`. UI: `/procurement/orders/[id]/receive` (server shell + heavy client form with per-line qty_received_now defaulted to pending + qty_rejected + rejection-reason required when rejected > 0 + batch/expiry/remarks; paperwork section: challan/vendor-invoice/vehicle/transporter/e-way/QC-status; "line will close" hint when receipt fulfils a line; save-as-draft vs save-and-post; over-receipt is allowed with amber chip), `/procurement/grns` (list with 3 status filter chips + accepted/rejected qty rollup per row), `/procurement/grns/[id]` (header with status + QC pill + linked PO + warehouse, lines table with accepted/rejected highlighted, paperwork card, notes card, draft-only Post/Cancel buttons via small client island), PO detail extended with "Receive goods" CTA (visible when PO is approved/sent/partly_received and any line is unfulfilled) + Goods receipts section listing GRNs against the PO + per-line "X/Y received" inline indicator under the description. `/procurement` landing replaced the DEL-017 gap marker with a live link to `/procurement/grns`. **Deferred to P1γ:** RTV (Return to Vendor) for posted GRNs, PO PDF + WhatsApp/email send. |
| DEL-018 | Procurement: Vendor Bill (AP) + 3-way match | Must-have post-C#2 | 📋 P2 | Vendor invoice + PO ↔ GRN ↔ invoice reconciliation. Toggles per `procurement.ap_master` (PLAT-028): `native` = CRMOS owns AP; `tally` = read-through. |
| DEL-019 | Procurement: AP ageing + dunning (mirror of AR) | Must-have post-C#2 | 📋 P2 | — |
...
| FIN-020 | MSME 45-day compliance + MSME-1 filing | Must-have post-C#2 | 📋 P2 | MSMED Act 2006: payment within 45 days of supply; 3× bank-rate interest penalty on default. Half-yearly MSME-1 filing for any pending >45-day dues. Vendor master flag already in place (REL-016). |
| FIN-021 | TDS computation (194Q/194C/194J/194I) + Form 16A + 26Q | Must-have post-C#2 | 📋 P3 | Per-section auto-detection at vendor-bill stage. PAN-availability check (PAN missing → 20% non-PAN rate). Quarterly 26Q return; annual Form 16A per vendor. |
| FIN-022 | Vendor payment scheduling + NEFT/RTGS export | Must-have post-C#2 | 📋 P3 | Payment release approval (reuses PLAT-014), bank file export (HDFC / ICICI / SBI formats), payment voucher, vendor ledger reconciliation. |
