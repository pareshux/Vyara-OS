...
export async function createQuotation(data: {
  ...
  // Allocate quotation_number from the tenant template + global
  // sequence (PLAT-010). If allocation returns null for any reason
  // we omit the field and the trg_quotation_number trigger fills
  // the Vyara default — never blocks creation on a code failure.
...
export async function updateQuotationStatus(
  ...
  if (dbStatus === 'sent') {
    // Emit quote.sent — signals downstream (WhatsApp PDF send in production via AiSensy)
    try {
      const { inngest } = await import('@/lib/inngest/client')
      await inngest.send({
        name: 'quote.sent',
        data: { quote_id: quotationId },
      })
    } catch (e) {
      console.error('Failed to emit quote.sent', e)
    }
  }
