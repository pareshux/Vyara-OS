...
import { createVehicleType, updateVehicleType } from '@/lib/actions/vehicle-types'
...
export function VehicleTypeForm({ mode, initial }: Props) {
  ...
  function submit() {
    ...
    startTransition(async () => {
      const res = mode === 'create'
        ? await createVehicleType({ code, label, sort_order: sortOrder })
        : await updateVehicleType(initial!.id, { label, sort_order: sortOrder })
    ...
    <>
      {mode === 'create' ? (
        <Button size="sm" onClick={() => setOpen(true)}>
          <PlusCircle className="size-4 mr-1.5" /> Add type
        </Button>
      ) : (
      ...
      <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) reset() }}>
        <DialogContent>
          <DialogHeader><DialogTitle>{mode === 'create' ? 'Add vehicle type' : 'Edit vehicle type'}</DialogTitle></DialogHeader>
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="label" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Car" />
              </div>
            </div>
