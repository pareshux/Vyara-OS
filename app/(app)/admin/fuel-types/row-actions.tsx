...
import { updateFuelType } from '@/lib/actions/fuel-types'

export function FuelTypeRowActions({
  ...
  function toggleActive() {
    if (isActive && usageCount > 0) {
      const ok = confirm(`Deactivate "${label}"? ${usageCount} vehicle${usageCount === 1 ? '' : 's'} currently use this fuel — they'll stay assigned, but the fuel won't appear in new-vehicle dropdowns.`)
      if (!ok) return
    }
    startTransition(async () => {
      const r = await updateFuelType(id, { is_active: !isActive })
  ...
  return (
    <div className="flex justify-end items-center gap-1">
      <FuelTypeForm mode="edit" initial={{ id, code, label, sort_order: sortOrder }} />
      <DropdownMenu>
